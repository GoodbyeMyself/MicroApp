...
function SM4Util() {
    ...
    this.encryptData_ECB = function(plainText) {
        try {
            ...
            var keyBytes = stringToByte(generateRandKey());
            sm4.sm4_setkey_enc(ctx, keyBytes);
            var encrypted = sm4.sm4_crypt_ecb(ctx, stringToByte(plainText));
            var cipherText = base64js.fromByteArray(keyBytes.concat(encrypted));
    ...
    generateRandKey = function() {
        var min = 0x21, max = 0x7e;
        var key = "";
        for(var i = 0; i < 16; i++) {
            key += String.fromCharCode(parseInt(Math.random()*(max-min+1)+min,10));
        }
        return key;
    }
...
DigestUtils = {
    encrypt:function(plainText) {
        return (new SM4Util()).encryptData_ECB(plainText);
    }
}